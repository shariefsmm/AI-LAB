...
class agent:
	...
	def update(self):
		...
		# checks if exit time of first path is time of environment 
		if self.exit==s.time:
			if self.active==2:
				self.active=1
				self.update_exit()
		...
		# checks if exit time of current path is time of environment 
		if(self.exit==s.time):
...
# list for priority queue to maintain exit time for each vehicle
h=[]
heapq.heapify(h)

# create vehicle object for each vehicle
while i<m:
...
# update vehicle object at each pop of h
while h:
