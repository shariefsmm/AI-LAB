...
class Heurestic:
	...
	def action(self,x,y,g):
		if x<s.n-1 and self.v[x+1][y]==0 and s.grid[x+1][y]==0:	# right
			...
			heapq.heappush(self.h,(g+1+distance,(x+1,y,g+1,(x,y))))
		...
		if y<s.n-1 and self.v[x][y+1]==0 and s.grid[x][y+1]==0:	# down
			...
			heapq.heappush(self.h,(g+1+distance,(x,y+1,g+1,(x,y))))
		...
		if x>0:
			if y<s.n-1 and self.v[x-1][y+1]==0 and s.grid[x-1][y+1]==0: # diag up-right
				...
				heapq.heappush(self.h,(g+1+distance,(x-1,y+1,g+1,(x,y))))
	...
	def BFS(self):
		...
		heapq.heappush(self.h,(self.Distance(0,0),(0,0,0,(-1,-1))))
		while self.h:
			(count,(x,y,g,P))=heapq.heappop(self.h)
			if self.v[x][y]==1:continue
			if self.v[x][y]==0:self.v[x][y]=1
			self.p[x][y]=P
...
class Manhattan(Heurestic):
	def __init__(self):
		print("Manhattan heuristic path: ")
		Heurestic.__init__(self)

	def Distance(self,x,y):
		return  (abs(x-(s.n-1))+abs(y-(s.n-1)))

class Euclidian(Heurestic):
	def __init__(self):
